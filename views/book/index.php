...
//var_export($type);
//exit();
$this->pageTitle=Yii::app()->name . ' - Книжная полка';
	...
	<div class="hero-container" id="page">
		<div class="row-fluid">
			<div class="span7 offset1">
				<div>
					<h1 class="section-form-heading">Книжная полка</h1>
					<h2 class="section-hero-description-volonteer">
                           Здесь представлены книги, легкие для восприятия не обладающего психологическими знаниями читателя и полезные ему
                    </h2>
				</div>
			</div>
		</div>
...
<div class="container">
  <?php
                echo  "<ul class='section-book-item'>";
                     
                        if (!empty($book)){
                           foreach($book as $row){
                          echo "<div class='place_of_pic'>";
                               echo "<br>";
                               echo  "<div class='site'>".CHtml::decode($row['site'])."</div>";
                               echo  "<div class='section-book-item-name'>".CHtml::decode($row['title'])."</div>";
                               echo  "<div class='section-book-item-description'>".CHtml::decode($row['author'])."</div>";
                               echo "<div class='descriptionBtn'>"; ///вокруг кнопки
                                    echo "<button class='js-open-modal' data-modal='".CHtml::decode(($row['id']))."'>
                                       Описание книги
                                    </button>";  
                              echo "</div>";//вокруг кнопки
                                ...
                                $link = trim($row['link']);
                                echo "<a href='$link' target=\"_blank\">Читать</a>";
                                echo "</div>"; //div classs modal
                               
                                            
                                
                          echo "</div>";//place_of_pic
                           } 
